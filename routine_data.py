    ...
    for cc in range(len(country)):
        ...
        if IDf == 'QM':
            ...
            for key in ['TR', 'Class', 'Name', 'Aggregation', 'Frequency']:
                if key in spec:
                    # Keep filtered metadata as numpy arrays to preserve vectorised comparisons later on
                    spec[key] = np.array([val for val, freq in zip(spec[key], spec['Frequency']) if freq == 'Q'])
            ...
            for key in ['TR', 'Class', 'Name', 'Aggregation', 'Frequency']:
                if key in spec:
                    spec[key] = np.array([val for val, freq in zip(spec[key], spec['Frequency']) if freq == 'M'])
        else:
            spec['agg'] = 0
